...
export function apiGet<T>(uri: string, params: any): Promise<T> {
	return api<T>(uri, 'get', undefined, params);
}
...
export function apiPost<T>(uri: string, data: any, onUploadProgress?: (progressEvent: any) => void): Promise<T> {
	return api<T>(uri, 'post', data, undefined, onUploadProgress);
}
...
async function api<T>(
	...
	return call(uri, method, data, params, onUploadProgress);
}
